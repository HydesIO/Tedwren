...
/// singleton so timesheets persist across requests within the host. Seeded with a small demo week under a
/// fixed demo company so the valuation-period view (MC-24) returns data without any attendance being captured.
/// </summary>
public sealed class InMemoryTimesheetStore
{
    /// <summary>The fixed demo company the seed timesheets belong to.</summary>
    public static readonly Guid DemoCompanyId = Guid.Parse("44444444-4444-4444-8444-000000000001");

    /// <summary>The fixed demo site used by the seed lines.</summary>
    public static readonly Guid DemoSiteId = Guid.Parse("44444444-4444-4444-8444-000000000010");
    ...
    public InMemoryTimesheetStore(bool seed)
    {
        ...
        Seed("M. Adeyemi", TimesheetStatus.Submitted, new[] { 8m, 8m, 7.5m, 8m, 6m });
        Seed("J. Fletcher", TimesheetStatus.Approved, new[] { 8m, 8m, 8m, 8m, 8m });
        Seed("R. Okafor", TimesheetStatus.Draft, new[] { 7m, 8m, 8m });
    }
    ...
    private void Seed(string operative, TimesheetStatus status, IReadOnlyList<decimal> dailyHours)
    {
        var timesheet = new Timesheet
        {
            ...
            SubmittedUtc = status is TimesheetStatus.Submitted or TimesheetStatus.Approved
                ...
                : null,
            DecidedBy = status is TimesheetStatus.Approved ? "Priya Shah" : null,
